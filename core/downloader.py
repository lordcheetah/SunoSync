...
class SunoDownloader:
    ...
    def download_single_song(self, clip, directory, headers, token, existing_uuids, rate_limiter, subfolder_name=None):
        ...
        ext = file_ext or ".mp3"
        # build_safe_path budgets for the *whole* path, not just the filename:
        # organize_by_month and organize_by_track each add a directory level.
        out_path = build_safe_path(target_dir, title, ext)
